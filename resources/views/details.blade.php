        ...
        <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-5 mt-5">
            <div class="col-span-2 bg-white shadow-lg rounded p-5">
                ...
                <div class="mt-5 ">
                    ...
                    <input id="name" type="hidden" value="{{$place->address}}">
            ...
            <div class="bg-white shadow-lg rounded p-4 h-52">
                ...
                @auth
                    <a href="{{route('bookmark',$place->id)}}" class="border border-teal-500 text-xs text-teal-500 hover:bg-teal-500 hover:text-gray-100 rounded ml-3 p-1">
                        <span class=""><i class="fa {{Auth::user()->alreadyBookmark($place->id) ? 'fa-bookmark':'fa-bookmark-o'}}"></i></span> علامة مرجعية 
                    </a>
            ...
            <div class="bg-white col-span-2 shadow-lg rounded p-5 grid grid-cols-1 lg:grid-cols-3">
                <div class="text-center v_line">
                    <h1 class="rating">
                        @include('includes.rating', ["count" => $total])
                    ...
                    <div class="review-block ">
                        <div class="grid grid-cols-3 p-5">
                            ...
                            <div class="col-span-2">
                                <div class="rating"> 
                                   @include('includes.rating', ["count" => $review->avgRating()])
                                </div>
                                ...
                                <div class="mt-3">
                                    @auth
                                            ...
                                            <button id="like" type="button" data-id="{{$review->id}}" class="border rounded p-1 text-xs like">
                                                 {!! Auth::user()->alreadyliked($review->id) ? '<i class="fa fa-thumbs-down"><small>الغاء اعجاب </small></i>' : '<i class="fa fa-thumbs-up"><small>اعجبني </small></i>' !!}
                                                <span>{{$review->likes_count}}</span>
                                            </button>    
                                        
                                  
                                    @else
                                    <span class="border rounded text-xs p-1">
                ...
                <form class="form-contact" action="{{route('review.store')}}" method="post">
                    ...
                    <div class="grid grid-cols-2 mt-5">
                        ...
                        <div class="">
                            <div class="rating float-right">
                                <h5>الجودة</h5>
                                <input type="radio" id="rating_quality1" name="quality_rating" value="5" />
                                <label for="rating_quality1" title="ممتاز"></label>
                                <input type="radio" id="rating_quality2" name="quality_rating" value="4" />
                                <label for="rating_quality2" title="جيد جدًا"></label>
                                <input type="radio" id="rating_quality3" name="quality_rating" value="3" />
                                <label for="rating_quality3" title="متوسط"></label>
                                <input type="radio" id="rating_quality4" name="quality_rating" value="2" />
                                <label for="rating_quality4" title="سيء"></label>
                                <input type="radio" id="rating_quality5" name="quality_rating" value="1" />
                                <label for="rating_quality5" title="سيء للغاية"></label>
                            </div>
                        </div>
                    </div>
    ...
    $(function(){
        $('button.like').on('click', function(){
            ...
            $.ajax({
                ...
                if(data){
                    $.trim(likeBtn.find('small').text()) === "اعجبني" ? showUnlike(data):showlike(data)

                }else{
                    alert('لايمكنك الاعجاب');
                }
            }); 

            function showUnlike(count){
                likeBtn.html('<li class="fa fa-thumbs-down"></li> <small>' + 'الغاءاعجاب' + '</small>' + count);
            }

            function showlike(count){
                likeBtn.html('<li class="fa fa-thumbs-up"></li> <small>' + 'اعجبني' + '</small>' + count);
            }
        });
    });
